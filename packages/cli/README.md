...
> Test cases are written in TypeScript and run with [Vitest](https://vitest.dev)
> rather than a bespoke CLI command — see [Testing](#testing) below.
...
[Vitest](https://vitest.dev) using the
**[`@rolaca11/mcp-inspector-vitest`](../vitest/README.md)** package: a
`defineMcpTest()` fixture that owns the session lifecycle, a typed client
returning ergonomic result objects, and MCP-aware `expect` matchers (`toBeOk`,
`toBeMcpError`, `toHaveText`, `toHaveStructured`, …).
...
Because it's just Vitest, CI reporters (JUnit, TAP, …), watch mode, `-t`
filtering, and `--coverage` all come for free. See the
[`@rolaca11/mcp-inspector-vitest` README](../vitest/README.md) for the full
primitive and matcher reference, unit-testing with an injected `connect`, and
named-`.mcp.json`-target setup.
